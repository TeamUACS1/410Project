...
def addFriend(request):
	...
	if(request.method == 'POST'):
		...
		if (request.POST.get("delete", "") == "1"):
			post_friend = Friends.objects.filter((Q(authorguid1=request.session['user_guid'])&Q(authorguid2=username2))|(Q(authorguid2=request.session['user_guid'])&Q(authorguid1=username2)))
			post_friend.delete()
		else:
...
def followUser(request):
	...
	if(request.method == 'POST'):
		username2 = request.POST.get("ID", "")
		if (request.POST.get("delete", "") == "1"):
			post_follow = Follows.objects.filter((Q(authorguid1=request.session['user_guid'])&Q(authorguid2=username2)))
			post_follow.delete()
		else:
...
def viewfriend(request, author_guid):
	...
	for pending in pendingRequests:
		if (request.session['user_guid'] == pending.authorguid1):
			friendflag = 1
		setattr(pending, 'authorName', Authors.objects.filter(guid=pending.authorguid1)[0].displayname)
	pended = Friends.objects.filter(Q(authorguid2=request.session['user_guid'])&Q(accepted=str(0)))
	for pending in pended:
		if (author_guid == pending.authorguid1):
			friendflag = 1
	...
	for follow in followers1:
		if (follow.authorguid2 == request.session['user_guid']):
			followingflag = 1
	...
	for follow in following2:
		if (follow.authorguid1 == request.session['user_guid']):
			followerflag = 1
	...
	for things in friends:
		if (things.guid == request.session['user_guid']):
			friendflag = 1
	...
	return render_to_response('main/friend.html', {'friends': friends,"posts":posts, "userProfile": userProfile, "pendingRequests": pendingRequests, "following":following, "followers":followers, "followerflag": followerflag, "followingflag":followingflag, "friendflag":friendflag}, context)
